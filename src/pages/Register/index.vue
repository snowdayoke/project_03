...
<script>
  export default {
    ...
    methods: {
      ...
      async getCode(){
       try {
        this.phone && await this.$store.dispatch('getCodeList',this.phone)
      ...
      // 完成注册
     async userRegister(){
        const success = await this.$validator.validateAll();  //全部表单验证成功|失败  返回布尔值 
        // 只有全部表单验证成功，才能再想服务器发请求，进行注册
        if(success){
           try {
